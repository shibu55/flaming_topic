...
def search_topic(title, file_name, sg, vector_size, min_count, window, process):
    ...
    for k in range(10):
        ...
        results = []
        for item in model.wv.most_similar(word, topn=30):
            if len(item[0])>1:
                results.extend(item)
        ...
        for i, j in zip(it, it):
            rank_words.append(i)
        # return calculate_by_dcg(rank_words[0:15], title)
        hit, dcg = calculate_by_dcg(rank_words[0:15], title)
        # print(hit, dcg)
        total_hit += hit
        total_dcg += dcg

    return total_hit/10, total_dcg/10
...
def calculate_by_dcg(rank_words, title):
    ...
    text_word_set = set(wakati.split(' '))
    # print(text_word_set)
    result = 0
    hit_count = 0
...
def main():
    ...
    cf = csv_file('result-all-10-700.csv')
        ...
        for min_count in min_count_list:
            for window in window_list:
                ...
                for i in range(len(file_names_all)):
                    res = search_topic(titles_all[i], file_names_all[i], sg, vector_size, min_count, window, 'b')
                    sum_hit += res[0]
                    sum_dcg += res[1]

                ave_hit = sum_hit/len(file_names_all)
                ave_dcg = sum_dcg/len(file_names_all)
                stop = time.time()
                ...
                cf.write([vector_size, min_count, window, round(sum_hit, 1), round(sum_dcg), round(ave_hit, 2), round(ave_dcg, 1), round(stop-start)])
                print('%s sg = %d, vector_size = %d, min_count = %d, window = %d' % (t, sg, vector_size, min_count, window))
